 // Register the plugin (Required for GSAP v3)
    ...
    // Login button loading state from login.ejs
    const loginForm = document.getElementById("loginForm");
    const loginBtn = document.getElementById("login");
    if (loginForm && loginBtn) {
        loginForm.addEventListener("submit", function () {
            loginBtn.innerHTML = `<span style="display:inline-block;width:18px;height:18px;border:3px solid rgba(255,255,255,.45);border-top-color:#fff;border-radius:50%;vertical-align:-3px;margin-right:8px;animation:spinLogin .7s linear infinite;"></span> Creating Account...`;
    ...
    spinStyle.innerHTML = `@keyframes spinLogin{to{transform:rotate(360deg)}}`;
